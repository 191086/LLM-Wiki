...
- **完整算法**（§5，式 9–12、Algorithm 1）：组合目标 $L^{CLIP+VF+S} = L^{CLIP} - c_1 L^{VF} + c_2 S$（值函数平方误差 + 熵奖励）；优势用**截断 GAE**（式 11–12，$\lambda=1$ 时退化为有限时域估计式 10；估计器本尊见 [[gae]] / [[gae-paper]]）；Algorithm 1：N 个并行 actor 各采 T 步 → 在 NT 样本上做 K epochs 小批量 SGD → $\theta_{old} \leftarrow \theta$。MuJoCo 基准超参：T=2048、10 epochs、minibatch 64、$\gamma=0.99$、$\lambda=0.95$（Table 3）
...
- [[gae]]（优势估计器本尊：式 11–12 的出处，γ/λ 偏差-方差语义与推导）｜ [[gae-paper]]（GAE 原文）
