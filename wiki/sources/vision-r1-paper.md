...
- **[[rule-based-reward|准则驱动奖励函数]]**：三路奖励求和——dual format（模板 ∧ 内容校验，全过才给 1）、recall（有效预测数 / GT 数）、precision（有效预测平均 IoU）；匹配用 **box-only 简化匈牙利匹配**（LVLM 输出确定性类别标签、无类别概率，类别项帮助小：42.1 vs 41.9 mAP，§4.3 Table 3）
- **[[rule-based-reward|渐进式规则收紧]]**：differentiation（低于 ξ₁ 记 0、高于 ξ₂ 给满、中间线性，拉开组内奖励差距）+ staged progression（阈值 0.5/0.5/0.75 → 0.75/0.75/0.9）持续加压，防 reward hacking（§3.3）
