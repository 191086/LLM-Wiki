...
## [2026-09-16] lint | 公式编号补全、行内公式提升与 sources 页公式锚点修复

- 起因：用户发现「很多公式没有编号但正文多处引用」。摸底：全库约 225 处「式 N / Eq.N」引用中约 90 处指向**本页内展示却无编号**的行间公式；10 个概念页共 35 个行间公式仅 1 个带 `\tag`（mpo 式 12）。根因：约定了「按原论文公式号引用」（原文定位），从未约定「展示公式要带可见编号」
- **概念页 `\tag` 补全（9 页 24 块）**，编号一律用原论文公式号、写法与该页正文引用一致：[[trpo]]（式 1/3/8/9）、[[ppo]]（式 6/7/9/11（修正）/12）、[[dpo]]（式 4/5/7）、[[rlhf]]（式 1 InstructGPT、式 3 dpo-paper）、[[rope]]（式 11/14–15/16/18/31–33/34/37）、[[grpo]]（Eq.1/2）、[[reward-model]]（式 1 BT）、[[quad-data-curation]]（式 4/6）；教学推导 / 中间步 / 走查块不标
- **行内公式提升 6 组**（均对照 raw/ PDF 核验后写入）：[[trpo]] 式 5（混合策略，顺带把「新策略与旧策略混合」修正为原文口径「改进策略 $\pi'$ 与当前策略 $\pi_{old}$ 混合」）+ 式 6（CPI 下界）；[[ppo]] 式 8（自适应 KL 惩罚）；[[dpo]] 式 6（按原文 $1/(1+\exp(\cdot))$ 形态转录并注 margin 读法）；[[rlhf]] InstructGPT 式 2（PPO-ptx 目标，全库首次展示，注 $\gamma=0$ 即纯 PPO）；[[rope]] 式 1–2（三路投影 + softmax 注意力，原文形态）
- **sources 页公式锚点补全**（修复库内悬空引用的锚点链）：[[vision-r1-paper]] 补 Eq.1–8 全部内容锚点 + 页顶收录注记（式 6「平均 IoU」行文与求和指示子 / 分母 $M$ 的口径出入显式标注）；[[ostrakon-vl-paper]] 补式 1–16 分区锚（QUAD 式 1–7 逐条转录、OCL 式 8–11 投票与分层、MPO 式 12、ShopBench MG/ML/VNR/VIF 式 13–16 转录）+ 页顶收录注记
- 核验方式：pdftotext 定位 + pypdfium2 渲染 + DPO / InstructGPT 公式页高倍放大目检复核。**工具注记**：docling 的 uv 工具环境已不在（技能文档路径 `~/.local/share/uv/tools/docling` 失效、模型缓存已清），本次以 pdftotext + pypdfium2（/tmp 临时 venv）替代，待修技能文档
