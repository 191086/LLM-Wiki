...
3. **至少一个手工走查**——小输入完整跑一遍 / 公式数值代入 / 反例演示；没有走查视为没讲清机制
4. 新术语（含缩写）首次出现即解释或给一句话 gloss，不默认读者已知
5. 关键论断句末锚定 [[sources/…]]；wiki 外知识块引用标注 + 候选来源；示例数字经本机复算或原文核对
6. 表格只放对照型内容，叙述性内容写正文
...
- **展开优先于罗列**：具体实例（小例、走查、数字表、可运行片段）优先于 bullet 清单；正文编号分节、每节自包含，支持「见 [[页名]] §N」式引用，素材少时可从简
- 关键论断句末锚定 [[sources/…]]；实测数字须能在对应来源页查到或经本机复算；**wiki 外一般性知识显式标注**（块引用 + 候选来源），不与有来源支撑的内容混淆
- 表格只放对照型内容（方案 / 阶段 / 变体对比），叙述性内容写正文
- 概念页从严：手工走查为硬性要求、自查清单全过才记 log；详版范例从库中现有概念页里挑最成熟的作参考（供参考不供套用）
...
- 缺失的交叉引用
- 数据空白（值得用 web 搜索补齐的）
- 概念页达标情况：对照「写页自查清单」扫全库，揪出无走查、术语未解释、未编号分节等摘要式页面（大 ingest 后必查——批量写入最容易顺手写薄）
